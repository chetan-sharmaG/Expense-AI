...
export default function DashboardView({ state, onNavigate }: DashboardViewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 p-6 rounded-2xl text-white shadow-sm border border-slate-800 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          <div className="flex items-center gap-2 px-3 py-1 bg-green-500/10 text-green-400 rounded-full text-xs font-semibold shrink-0">
          ...
          <button 
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 transition-colors text-white text-sm font-medium rounded-lg shadow-md flex items-center gap-2 cursor-pointer"
          >
            <Layers className="size-4" />
            Launch WhatsApp Bot Simulator
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Total Spending */}
        <div className="bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-800 flex items-center gap-4 hover:border-slate-700 transition">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">Across all registered family groups</p>
          </div>
        </div>
        ...
        {/* Groups Active */}
        <div className="bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-800 flex items-center gap-4 hover:border-slate-700 transition animate-fade-in">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">{users.length} Family members total</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm lg:col-span-2 min-w-0 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs bg-slate-950 border border-slate-800 text-slate-400 px-2.5 py-1 rounded-lg flex items-center gap-1 font-medium font-mono">
              <TrendingUp className="size-3 text-indigo-400" /> INR (₹)
            </span>
          </div>
          ...
          <div className="h-56 sm:h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailyTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={2.5} fillOpacity={1} fill="url(#colorSpend)" name="Amount Spend" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm flex flex-col justify-between min-w-0 space-y-4">
          <div>
          ...
          <div className="h-56 relative flex items-center justify-center">
            ...
            {categorySpendData.length > 0 && (
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-[10px] text-slate-500 font-medium font-sans uppercase tracking-wider">Top Share</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm min-w-0 space-y-4">
          <div>
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm min-w-0 space-y-4 flex flex-col justify-between">
          <div>
          ...
          <div className="divide-y divide-slate-800 flex-1 mt-2">
                ...
                return (
                  <div key={user.name} className="py-3 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="size-8 rounded-full bg-slate-800 flex items-center justify-center font-bold text-slate-350 text-sm">
                        {user.name[0]}
                      </div>
                      <div>
                        <h4 className="text-sm font-semibold text-slate-200">{user.name}</h4>
                        <span className="text-[11px] text-slate-500">Contributor</span>
                      </div>
                    </div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-slate-500 font-mono font-medium">{Math.round(percentage)}% of family total</p>
                    </div>
                  </div>
                );
              })
